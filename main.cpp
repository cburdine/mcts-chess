...
int main(){
    ...
    loader.get_training_data(dataset, 0, 100);
    ...
    // auto game = ChessGame(
    //           make_unique<ChessNetAgent>(WHITE, "./jupyter/simple_chess_net_v2"), 
    //           make_unique<ChessNetAgent>(BLACK, "./jupyter/simple_chess_net_v2"));
   
    
    // have a player (white) play against the nnet:
    // auto game = ChessGame(
    //     make_unique<ChessPlayerAgent>(WHITE, cin),
    //     //make_unique<ChessPlayerAgent>(BLACK, cin));

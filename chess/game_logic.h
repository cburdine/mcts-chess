...
inline int prev_rcastle(move_vector m){ return m & PREV_RCASTLE; }
inline int prev_lcastle(move_vector m){ return m & PREV_LCASTLE; }
inline int prev_check(move_vector m){ return m & PREV_CHECK; }
...
inline void set_prev_en_passant_x(move_vector& m, int x){ m = (m & ~(7<<24)) | (x<<24); }
